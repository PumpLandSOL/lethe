...
async function ensureChain(eth) { try { await eth.request({ method: 'wallet_switchEthereumChain', params: [{ chainId: CHAIN_HEX }] }); } catch (e) { if (e && e.code === 4902) { try { await eth.request({ method: 'wallet_addEthereumChain', params: [{ chainId: CHAIN_HEX, chainName: 'Arc', nativeCurrency: { name: 'USDC', symbol: 'USDC', decimals: 18 }, rpcUrls: ['https://rpc.mainnet.arc.io'], blockExplorerUrls: ['https://explorer.arc.io'] }] }); } catch (e2) {} } } }
async function connectPhantom() {                                // EVM wallet on Arc (name kept for the call sites)
  const eth = evm();
...
function renderPanel() {
  ...
  if (tab === 'seal') {
    ...
    p.innerHTML = `<div class="note">Send <b>USDC on Arc</b> to the treasury and it is credited to your ledger once the receipt confirms. <b>Every deposited dollar sits in the treasury address</b> — see the on-chain balance in the Treasury bar below.</div>
      <div class="field"><input id="in" type="number" placeholder="50.00 minimum" min="50"><span class="u">USDC</span></div>
      <div class="kv"><span>Treasury</span><b>${M && M.treasury ? M.treasury.slice(0, 8) + '…' + M.treasury.slice(-6) : '—'}</b></div>
      <div class="kv"><span>Credited so far</span><b>${A ? fmt(A.deposited, 2) + ' USDC' : '—'}</b></div>
      <button class="btn wide" id="act" style="margin-top:14px">Send USDC from wallet</button>
...
async function sendUsdg(amount) {
  if (needWallet()) return; if (!amount || amount <= 0) return toast('enter an amount', true); const minDep = (M && M.minDeposit) || 50; if (amount < minDep) return toast('minimum deposit is ' + minDep + ' USDC', true);
  const eth = evm(); if (!eth) return toast('open a wallet to send USDC, or paste a tx hash', true);
  if (!M || !M.treasury || /^0x0{40}$/.test(M.treasury)) return toast('treasury not configured', true);
  try {
    await ensureChain(eth);
    ...
    for (let i = 0; i < 40; i++) { await new Promise((r) => setTimeout(r, 3000)); const r = await api('/api/deposit', { wallet, tx }); if (r.ok) { A = r; renderAccount(); loadMetrics(); return toast(`credited ${fmt(r.amt, 2)} USDC`); } if (r.error && !/pending|not found/.test(r.error)) return toast(r.error, true); }
