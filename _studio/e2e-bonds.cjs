// Bonds + vigil boost + LETHE payout queue E2E (dev server, DEV_FAUCET=1).
const B = 'http://localhost:8206'; const U = '0x0000000000000000000000000000000000000309';
...
(async () => {
  ...
  ok('bond 200 USDG → LETHE at discount', b.ok && f(b.bonded) === 200 && Math.abs(b.letheOut - 200 / (b.market * 0.8)) < 1e-6, `${Math.round(b.letheOut)} LETHE @ ${b.price}`);
  ok('USDG left ledger, nothing minted', f(b.usdg) === 800 && f(b.lusd) === 0);
  ...
  ok('reserve grew by 200 (over-collateralizes lUSD)', f(m1.collateralUsd - col0) === 200, 'Δ ' + f(m1.collateralUsd - col0));
  ...
  const cl = await post('/api/bond/claim', {}); ok('claim pays the vested sliver', cl.ok && cl.claimedLethe > 0 && cl.lethe > 0, 'claimed ' + cl.claimedLethe.toFixed(4));
  const wd = await post('/api/withdraw', { asset: 'LETHE', amount: cl.lethe }); ok('LETHE withdrawal queued', wd.ok && wd.queued.asset === 'LETHE' && f(wd.lethe) === 0, 'id ' + wd.queued.id);
  await post('/api/dev/faucet', { amount: 100 }); const wd2 = await post('/api/withdraw', { amount: 100 }); ok('USDG withdrawal still works (default asset)', wd2.ok && wd2.queued.asset === 'USDC');
